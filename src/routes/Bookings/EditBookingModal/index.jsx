...
const EditBookingModal = ({booking, submitUpdatedBooking, handleClose}) => {
    const [bookingInfo, setBookingInfo] = useState({
        ...
        clientService: booking.clientService.service,
        appointmentDate: `${booking.appointmentDate.split('T')[0]}`,
        appointmentTime: booking.appointmentTime,
        appointmentAddress: booking.appointmentAddress,
        totalPeople: booking.totalPeople,
        touchupRequired: booking.touchupRequired,
    ...
    const handleChange = (e) => {
        ...
        if (name === 'booking[touchupRequired]') {
            setBookingInfo({...bookingInfo, touchupRequired: value })
        ...
        if (name === 'booking[status]') setBookingInfo({...bookingInfo, status: value });
        if (name === 'booking[paymentStatus]') setBookingInfo({...bookingInfo, paymentStatus: value });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        submitUpdatedBooking({bookingInfo});

        document.getElementById('edit-booking-modal').close();
    };

    return (
        <dialog id='edit-booking-modal' className='modal' onClose={handleClose}>
            <div className='modal-box lg:max-w-3xl'>
                ...
                <form id='form' className='my-4 px-4 md:px-8 lg:px-0 lg:mx-auto font-libertinus tracking-widest' onSubmit={handleSubmit}>
                    ...
                    <fieldset className='fieldset lg:max-w-3/4 md:mx-auto lg:px-10'>
                        ...
                        <select name='booking[clientService]' value={`${bookingInfo.clientService}`} className='select validator w-full' onChange={handleChange} required>
                        ...
                        <input type='text' name='booking[appointmentAddress]' value={`${bookingInfo.clientService !== 'Studio Walk-in' && bookingInfo.appointmentAddress}`} onChange={handleChange} className='input validator w-full' placeholder='Appointment Address' required={bookingInfo.clientService === 'Studio Walk-in' && false} title='Please enter an address' disabled={bookingInfo.clientService === 'Studio Walk-in' && true} />
                        ...
                        <select name='booking[touchupRequired]' value={bookingInfo.touchupRequired} className='select validator w-full' onChange={handleChange} required>
                        ...
                        <select name='booking[downPayment]' value={bookingInfo.downPayment} className='select validator w-full' onChange={handleChange} required>
                        ...
                        <legend className='fieldset-legend text-base'>Appointment Status</legend>
                        <select name='booking[status]' value={bookingInfo.status} className='select validator w-full' onChange={handleChange} required>
                            <option key={1}>Pending</option>
                            <option key={2}>Confirmed</option>
                            <option key={3}>Completed</option>
                            <option key={4}>Cancelled</option>
                            <option key={5}>Moved</option>
                        </select>
                        <div className='validator-hint'>Please select appointment status</div>

                        <legend className='fieldset-legend text-base'>Payment Status</legend>
                        <select name='booking[paymentStatus]' value={bookingInfo.paymentStatus} className='select validator w-full' onChange={handleChange} required>
                            <option key={1}>Pending Confirmation</option>
                            <option key={2}>Paid</option>
                            <option key={3}>Partial</option>
                            <option key={4}>Not Paid</option>
                        </select>
                        <div className='validator-hint'>Please set payment status</div>
                   </fieldset>
